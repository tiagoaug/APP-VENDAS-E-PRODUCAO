...
# We want to replace from `  const [note, setNote] = useState('');` down to `  const [isPreviewLoading, setIsPreviewLoading] = useState(false);`
start_marker = "  const [note, setNote] = useState('');"
end_marker = "  const [isPreviewLoading, setIsPreviewLoading] = useState(false);"
...
if idx1 != -1 and idx2 != -1:
  ...
  React.useEffect(() => {
    ...
    content = content[:idx1] + state_replacement + content[idx2:]
    print("SUCCESS STATE")
else:
    print("FAILED STATE")

# 3. Inject Profile UI
footer_marker = "        {/* Footer Actions */}"
idx3 = content.find(footer_marker)
...
"""
    ...
    while content[i] == ' ':
        spaces += ' '
        i -= 1
    content = content[:idx3] + profile_ui + spaces + content[idx3:]
...
popup_marker = "      {/* Popup de gerenciamento dos Textos"
idx4 = content.find(popup_marker)
...
"""
    ...
    while content[i] == ' ':
        spaces += ' '
        i -= 1
    content = content[:idx4] + profile_popup_ui + spaces + content[idx4:]
    print("SUCCESS POPUP")
else:
    print("FAILED POPUP")
