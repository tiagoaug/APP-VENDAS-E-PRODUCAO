...
# Find the index of `<Hammer size={13} /> Emitir OS - {selected.length}`
idx = content.find("<Hammer size={13} /> Emitir OS - {selected.length}")
if idx != -1:
    ...
    if "Compartilhar Ficha" not in content[end_btn:end_btn+500]:
        ...
        with open('src/views/PCPView.tsx', 'w', encoding='utf-8') as f:
            f.write(content)
        print("Button injected successfully by index!")
    else:
