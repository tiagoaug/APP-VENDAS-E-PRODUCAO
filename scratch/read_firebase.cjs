    ...
    if (activeProductIds.has(p.id)) {
      ...
      p.variations?.forEach(v => {
        ...
        v.consumptions?.forEach(c => {
          const isEmb = c.name?.toLowerCase().includes('emborrachado') || 
                        c.materialId?.includes('emborrachado') || 
                        configs.find(co => co.id === c.materialId)?.name?.toLowerCase().includes('emborrachado');
          if (isEmb) {
            console.log(`    Consumption: ${c.name || 'unnamed'} (Mat: ${c.materialId}), Qty: ${c.quantity}, ignoreColor: ${c.ignoreColor}, ignoreQty: ${c.ignoreQuantity}, basis: ${c.consumptionBasis}`);
          }
        });
      });
    }
