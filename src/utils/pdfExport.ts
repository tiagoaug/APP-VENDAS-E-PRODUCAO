 ...
 */
/** Compartilha um PDF já pronto em base64 (sem prefixo data:) — usado pra PDFs gerados no
 * backend (ex.: etiqueta de envio + DANFE simplificado combinados, ver mergeBlingShippingDocuments
 * em functions/src/bling/sync.ts), quando não existe um jsPDF local pra passar pro sharePDF. */
export const sharePdfBase64 = async (base64: string, filename: string) => {
  ...
  if (Capacitor.getPlatform() === 'web') {
    ...
    try {
      ...
      console.error('Error sharing merged PDF:', error);
