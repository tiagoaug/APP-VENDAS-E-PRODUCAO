        ...
        switch (card.id) {
              ...
              <div key="customers" className={`p-6 rounded-[1.5rem] border shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] flex flex-col gap-4 ${isDarkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-100"}`}>
                ...
                <div className="flex justify-between items-center">
                  <div className={`flex border p-0.5 rounded-xl shadow-sm dark:shadow-none ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'}`}>
                    <button onClick={() => setCustomerDashboardTab('DEBITS')} data-guide-anchor="dash.customers.tabs" className={`px-3 py-1.5 rounded-lg text-[10px] font-medium tracking-wide normal-case transition-all flex items-center justify-center gap-1.5 ${customerDashboardTab === 'DEBITS' ? 'bg-slate-500 dark:bg-slate-600 text-white shadow-lg' : 'text-blue-950 dark:text-blue-300 hover:text-slate-600 dark:hover:text-slate-300'}`}>
                    ...
                    <button onClick={() => setCustomerDashboardTab('CREDITS')} className={`px-3 py-1.5 rounded-lg text-[10px] font-medium tracking-wide normal-case transition-all flex items-center justify-center gap-1.5 ${customerDashboardTab === 'CREDITS' ? 'bg-slate-500 dark:bg-slate-600 text-white shadow-lg' : 'text-blue-950 dark:text-blue-300 hover:text-slate-600 dark:hover:text-slate-300'}`}>
                ...
                <div className="relative">
                  <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600" size={14} />
                  <input type="text" placeholder="Buscar cliente..." className="w-full bg-slate-50 dark:bg-slate-950 border-none rounded-xl pl-9 pr-4 py-2.5 text-[10px] font-medium tracking-wide text-blue-950 dark:text-blue-300 placeholder:text-slate-300 dark:placeholder:text-slate-700" value={customerDebtsSearch} onChange={(e) => setCustomerDebtsSearch(e.target.value)} />
                </div>
                    ...
                    <>
                      {customersWithDebts.filter(item => item.person.name.toLowerCase().includes(customerDebtsSearch.toLowerCase())).map((item, idx) => (
                        <div key={`cust-debt-${item.person.id}-${idx}`} onClick={() => onNavigate(ViewType.REPORT_DETAILED, "relacionamento-cliente", item.person.id)} data-guide-anchor="dash.customers.itemDebito" className={`p-3 rounded-xl border cursor-pointer transition-colors ${isDarkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-slate-50 border-slate-100 hover:bg-slate-100'}`}>
                          ...
                          <p className="text-[10px] font-medium tracking-wide text-blue-950 dark:text-blue-300 mt-0.5">{item.pendingCount} {item.pendingCount === 1 ? 'venda pendente' : 'vendas pendentes'}</p>
                        </div>
                      ))}
                      {customersWithDebts.filter(item => item.person.name.toLowerCase().includes(customerDebtsSearch.toLowerCase())).length === 0 && <p className="text-[10px] font-medium tracking-wide text-center text-blue-950 dark:text-blue-300 py-4">Nenhum cliente com débito.</p>}
                    </>
                  ) : (
                    <>
                      ...
                      {customersWithCredits.filter(p => p.name.toLowerCase().includes(customerDebtsSearch.toLowerCase())).length === 0 && <p className="text-[10px] font-medium tracking-wide text-center text-blue-950 dark:text-blue-300 py-4">Nenhum cliente com crédito.</p>}
                    </>
                  )}
                </div>
          ...
          case "factory_config": {
            ...
            const factoryItems = [
              ...
              { label: 'Cadastro de Solados',   desc: 'Catálogo e moldes',           screen: 'MATRIZES'   as const, color: 'text-orange-600',  bg: 'bg-orange-50 dark:bg-orange-950/20'  },
