...
const CANDIDATES: NavCandidate[] = [
  { id: 'purchases', label: 'Compras', icon: <ShoppingCart size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'sales', label: 'Vendas', icon: <ShoppingBag size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'management', label: 'Gerenciamento', icon: <PackagePlus size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'production', label: 'Prod.', icon: <Factory size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'bling', label: 'Bling', icon: <Building2 size={15} />, requiredModule: 'bling', requiredModuleLabel: 'Bling' },
  { id: 'entregas', label: 'Entregas', icon: <Truck size={15} />, requiredModule: 'entregas', requiredModuleLabel: 'Entregas' },
  { id: 'financial', label: 'Finan.', icon: <DollarSign size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'personal', label: 'Pessoal', icon: <UserIcon size={15} />, requiredModule: 'personal', requiredModuleLabel: 'Pessoal' },
  { id: 'rh', label: 'RH', icon: <UserCog size={15} />, requiredModule: 'rh', requiredModuleLabel: 'RH' },
  { id: 'pcp', label: 'PCP', icon: <GanttChartSquare size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'stock', label: 'Estoque', icon: <Boxes size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'people', label: 'Pessoas', icon: <Users size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'reports', label: 'Relatórios', icon: <BarChart3 size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'soleStock', label: 'Solados', icon: <Footprints size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'engineering', label: 'Engenharia', icon: <Database size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'purchaseNeeds', label: 'Necessidades', icon: <AlertTriangle size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'ruleOfThree', label: 'Regra de Três', icon: <Calculator size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'labelPrintStudio', label: 'Ajustes de PDF e JPG', icon: <Printer size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'catalogRequests', label: 'Pedidos de Catálogo', icon: <Inbox size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'sendCatalog', label: 'Enviar Catálogo', icon: <Link2 size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'fornecedores', label: 'Prestadores Terceirizados', icon: <Handshake size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'products', label: 'Produtos Cadastrados', icon: <Package size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'paymentMethods', label: 'Meios de Recebimento', icon: <CreditCard size={15} />, requiredModule: 'sales', requiredModuleLabel: 'Vendas' },
  { id: 'ocr', label: 'Extrator de Texto (OCR)', icon: <ScanText size={15} />, requiredModule: 'any', requiredModuleLabel: 'Qualquer' },
  { id: 'aiAssistant', label: 'Assistente de IA', icon: <Sparkles size={15} />, requiredModule: 'ai', requiredModuleLabel: 'IA' },
  { id: 'cuttingKnives', label: 'Facas de Corte', icon: <Scissors size={15} />, requiredModule: 'production', requiredModuleLabel: 'Produção' },
  { id: 'dashboardConfig', label: 'Organizar Dashboard', icon: <Layout size={15} />, requiredModule: 'any', requiredModuleLabel: 'Qualquer' },
  { id: 'accessibility', label: 'Acessibilidade e Personalização', icon: <SlidersHorizontal size={15} />, requiredModule: 'any', requiredModuleLabel: 'Qualquer' },
  { id: 'visualSetup', label: 'Assistente de Personalização Visual', icon: <Wand2 size={15} />, requiredModule: 'any', requiredModuleLabel: 'Qualquer' },
  { id: 'headerSpace', label: 'Espaço no Topo', icon: <Smartphone size={15} />, requiredModule: 'any', requiredModuleLabel: 'Qualquer' },
];
...
function NavRow({ item, isHidden, isPinned, isDarkMode, onToggleHidden, onTogglePinned, compact, onPointerDown, isDragging, dragStyle }: NavRowProps) {
  return (
    <motion.div
      ...
      style={dragStyle}
      className={`relative flex flex-col items-center gap-0.5 rounded-xl border select-none touch-none ${isDragging ? 'cursor-grabbing shadow-2xl scale-105 z-50' : 'cursor-grab transition-colors'} ${compact ? 'p-1' : 'p-1.5'} ${isHidden ? 'opacity-40' : ''} ${isDarkMode ? 'bg-slate-800/40 border-slate-700' : 'bg-slate-50 border-slate-100'} ${isDragging ? (isDarkMode ? 'bg-slate-800' : 'bg-white') : ''}`}
    >
      {!isHidden && (
        <button
          type="button"
      ...
      <button
        ...
        className={`absolute -bottom-1.5 -right-1.5 z-10 w-5 h-5 rounded-full flex items-center justify-center shadow transition-all ${isHidden ? (isDarkMode ? 'bg-slate-700 text-slate-400' : 'bg-white text-slate-400') : 'bg-indigo-600 text-white'}`}
      >
        {isHidden ? <EyeOff size={11} /> : <Eye size={11} />}
      </button>
      <div className={`${compact ? 'w-4 h-4' : 'w-5 h-5'} rounded-md flex items-center justify-center shrink-0 ${isDarkMode ? 'bg-slate-900 text-slate-300' : 'bg-white text-slate-600 shadow-sm'}`}>
        {item.icon}
      </div>
...
export default function BottomNavConfigModal({ isOpen, onClose, config, onSave, modulesConfig, isDarkMode }: BottomNavConfigModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[70000] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-4 min-h-0">
          <div className={`relative mx-auto max-w-[400px] w-full rounded-[2.3rem] p-2.5 shadow-xl ${isDarkMode ? 'bg-slate-700' : 'bg-slate-900'}`}>
              ...
              <div>
                ...
                <div className={`flex items-stretch gap-1.5 p-1.5 rounded-2xl ${isDarkMode ? 'bg-slate-900' : 'bg-white shadow-sm'}`}>
                  ...
                  <div className="flex flex-col items-center justify-center gap-0.5 w-12 shrink-0 rounded-xl bg-black/5 dark:bg-white/10 py-1.5">
                    <Settings size={15} className={isDarkMode ? 'text-slate-300' : 'text-slate-600'} />
