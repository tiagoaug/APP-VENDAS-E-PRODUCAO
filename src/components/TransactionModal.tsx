...
export default function TransactionModal({ 
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      prevTransactionIdRef.current = undefined;
      return;
    }

    const transactionChanged = transaction?.id !== prevTransactionIdRef.current;
    if (isInitialized.current && !transactionChanged) return;
  ...
  return (
    <>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm overscroll-contain">
      <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-[2rem] shadow-2xl overflow-hidden flex flex-col max-h-[85vh] border border-slate-200/50 dark:border-slate-800/80">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-5 pt-2 space-y-3 custom-scrollbar">
          ...
          <div className={`p-1.5 rounded-[2rem] border transition-all duration-300 flex gap-1 shadow-inner ${
            isDarkMode 
              ...
              : 'bg-white border-slate-200/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.06)]'
          }`}>
            <button
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Nº de Indicação</label>
            </div>
            <div className={`flex items-center gap-3 rounded-[1.3rem] border transition-all ${
              isDarkMode 
                ...
                : 'bg-white border-slate-200/60 shadow-[inset_0_2px_4px_rgba(0,0,0,0.06)] pr-3 focus-within:border-indigo-500/50 focus-within:ring-4 focus-within:ring-indigo-500/10'
            }`}>
              <input
                type="text"
                className={`flex-1 bg-transparent border-none rounded-[1.1rem] py-2.5 px-4 text-xs font-bold focus:ring-0 outline-none dark:text-white placeholder:text-slate-400 ${!isManual ? 'opacity-40 cursor-not-allowed' : ''}`}
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl shrink-0 transition-all shadow-sm ${
                  isManual 
                    ...
                    : 'bg-white border border-slate-200 shadow-sm dark:bg-slate-700 dark:border-transparent text-slate-500 dark:text-slate-400'
                }`}
              >
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Vínculo (Cliente/Fornecedor)</label>
            </div>
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
              ...
              <div className="flex items-center gap-2">
                <div className={`w-6 h-6 rounded-lg flex items-center justify-center transition-all ${
                 ...
                 <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">
                  Detalhamento de Itens {items.length > 0 && `(Total: R$ ${itemsTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })})`}
                </label>
              </div>
            </div>
            ...
            {items.length > 0 && (
              <div className="space-y-2">
                {items.map((item) => (
                  <div key={item.id}>
                    <div className={`flex gap-2 items-center p-2.5 rounded-[1.1rem] border ${
                      isDarkMode
                      ...
                      <div className="flex-1 space-y-2">
                        <input
                          ...
                          className="w-full bg-white dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-700/50 rounded-lg px-3 py-2 text-[10px] font-black uppercase tracking-tight dark:text-white placeholder:text-slate-350 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none"
                        />
                        <div className="flex items-center gap-2 px-1">
                          <span className="text-[9px] font-black text-indigo-500/50 uppercase tracking-[0.2em]">R$</span>
                          <input
                            type="number"
                            step="0.01"
                            value={item.amount || ''}
                            ...
                            placeholder="0,00"
                            className="w-full bg-transparent border-none outline-none p-0 text-xs font-black dark:text-white placeholder:text-slate-300 focus:ring-0 [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                          />
                        </div>
                      </div>
            ...
            {/* Botão de Adicionar Item estilizado como subcard abaixo dos itens */}
            <button
              type="button"
              onClick={addItem}
          ...
          {items.length === 0 && (
            <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Valor (R$)</label>
              </div>
              <div className="relative" ref={calculatorRef}>
                <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-655" size={16} />
                <input
                  type="number"
                  step="0.01"
                  className={`w-full bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-[1.1rem] py-2.5 pl-10 pr-10 text-sm font-black focus:ring-4 focus:ring-indigo-500/10 transition-all dark:text-white placeholder:text-slate-350 outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:border-indigo-500/50`}
                  placeholder="0"
                  value={amount}
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Data</label>
            </div>
            <div className="relative">
              <DatePicker
                value={date}
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Conta</label>
            </div>
            <div className="relative">
              <select
                className="w-full bg-slate-100/70 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-850 rounded-[1.3rem] py-3 pl-5 pr-9 text-[10px] font-black uppercase tracking-widest focus:ring-4 focus:ring-indigo-500/10 transition-all dark:text-white appearance-none outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.03)] focus:border-indigo-500/50"
                value={accountId}
          ...
          <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
            ...
            <div className="flex items-center gap-2">
              ...
              <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Status</label>
            </div>
          ...
          {type === TransactionType.EXPENSE && (
            <div className={`p-3 rounded-[1.3rem] border transition-all duration-300 flex flex-col gap-2 ${
              ...
              <div className="flex items-center gap-2">
                ...
                <label className="text-[9px] font-black uppercase tracking-[0.12em] text-slate-400">Recorrência</label>
              </div>
                  ...
                  {isRecurring && (
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 rounded-[1.1rem] py-2.5 px-4 text-xs font-black focus:ring-4 focus:ring-cyan-500/10 transition-all dark:text-white outline-none shadow-[inset_0_2px_4px_rgba(0,0,0,0.06)]"
                      />
    ...
    {activeCalculatorId && (
      <CalculatorPopover
        onApply={(val) => {
          if (activeCalculatorId === 'main') {
            setAmount(val);
          } else {
            updateItem(activeCalculatorId, 'amount', val);
          }
