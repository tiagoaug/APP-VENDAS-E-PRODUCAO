...
pattern = r"(<Hammer size=\{13\} /> Emitir OS - \{selected\.length\}.*?</button>)"
...
if match:
    ...
    if "Compartilhar Ficha" not in content[match.end():match.end()+500]:
        content = content[:match.end()] + button_statement + content[match.end():]
